...
class OrderUpdateAddressService extends ServiceBase
{
    /**
     * @var Order
     */
    ...
    public function beforeExecute()
    {
        ...
        $returnData = $addressManager->getUsableAddress($this->newOrder->getProvince(), $this->newOrder->getCity(),
            $this->newOrder->getDistrict());
        if (!$returnData->isSuccess()){
            return $returnData;
        }
        $this->newOrder->setDistrict($returnData->getData());
    ...
    public function execute()
    {
        ...
        if ((int)$this->newOrder->getDistrict() === 0 ){
